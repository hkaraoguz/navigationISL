...
// Alpha, Beta
void NavigationController::fcn_A_B(int numOfRobots, mpfr_t A, mpfr_t B, double b_[][4], double bt_[][3],  double brs_[][4], double ro)
{
    //mpfr_t A, B;
    mpfr_t temp;
    mpfr_rnd_t MPFR_RND;
    ...
    */
    ...
    for(int i=2;i<=numOfRobots;i++)
    {
        double  dist=-1;

        if (b_[i][3]!=0)
        {
            dist = abs(pow((b_[1][1] - b_[i][1]),2) + pow((b_[1][2] - b_[i][2]),2) -pow((b_[1][3]+b_[i][3]),2));
            /*
            if (dist<0)
            {
                int dedede=1;
        ...
        {
            dist = abs(pow((b_[1][1] - brs_[i][1]),2) + pow((b_[1][2] - brs_[i][2]),2) -pow((b_[1][3]+brs_[i][3]),2));
        }
...
void NavigationController::fcn_dAdx(mpfr_t dAdx, mpfr_rnd_t MPFR_RND, double b_[][4], double bt_[][3])
{
    mpfr_set_d(dAdx, 2*(b_[1][1]-bt_[1][1]), MPFR_RND);
}


void NavigationController::fcn_dAdy(mpfr_t dAdy, mpfr_rnd_t MPFR_RND, double b_[][4], double bt_[][3])
{
    mpfr_set_d(dAdy, 2*(b_[1][2]-bt_[1][2]), MPFR_RND);
}


void NavigationController::fcn_dBdx(int numOfRobots, mpfr_t dBdx, mpfr_t B, mpfr_rnd_t MPFR_RND, double b_[][4], double brs_[][4], double ro)
{
    mpfr_t temp;
    mpfr_init (temp);
    ...
    for(int i=2;i<=numOfRobots;i++)
    {
        ...
        if (b_[i][3]!=0)
        {
            val = 2*(b_[1][1]-b_[i][1])/( pow((b_[1][1] - b_[i][1]),2) + pow((b_[1][2] - b_[i][2]),2) - pow((b_[1][3]+b_[i][3]),2) );
        ...
        {
            val = 2*(b_[1][1]-brs_[i][1])/( pow((b_[1][1] - brs_[i][1]),2) + pow((b_[1][2] - brs_[i][2]),2) - pow((b_[1][3]+brs_[i][3]),2) );
            mpfr_set_d(temp, val, MPFR_RND);
            mpfr_add(dBdx, dBdx, temp, MPFR_RND);
        }
    }

    double val = (-2*b_[1][1])/ (pow((ro - b_[1][3]),2) - b_[1][1]*b_[1][1] - b_[1][2]*b_[1][2] ) ; // bounded
    mpfr_set_d(temp, val, MPFR_RND);
    mpfr_add(dBdx, dBdx, temp, MPFR_RND);
...
void NavigationController::fcn_dBdy(int numOfRobots, mpfr_t dBdy, mpfr_t B, mpfr_rnd_t MPFR_RND, double b_[][4], double brs_[][4], double ro)
{
    mpfr_t temp;
    mpfr_init (temp);
    ...
    for(int i=2;i<=numOfRobots;i++)
    {
        ...
        if (b_[i][3]!=0)
        {
            val = 2*(b_[1][2]-b_[i][2])/( pow((b_[1][1] - b_[i][1]),2) + pow((b_[1][2] - b_[i][2]),2) - pow((b_[1][3]+b_[i][3]),2) );
            mpfr_set_d(temp, val, MPFR_RND);
            mpfr_add(dBdy, dBdy, temp, MPFR_RND);
        }
        else if (brs_[i][3]!=0)
        {
            val = 2*(b_[1][2]-brs_[i][2])/( pow((b_[1][1] - brs_[i][1]),2) + pow((b_[1][2] - brs_[i][2]),2) - pow((b_[1][3]+brs_[i][3]),2) );
            mpfr_set_d(temp, val, MPFR_RND);
            mpfr_add(dBdy, dBdy, temp, MPFR_RND);
        }
    }
    double val = (-2*b_[1][2])/( pow((ro - b_[1][3]),2) - b_[1][1]*b_[1][1] - b_[1][2]*b_[1][2] ) ; // bounded
    mpfr_set_d(temp, val, MPFR_RND);
    mpfr_add(dBdy, dBdy, temp, MPFR_RND);
...
void NavigationController::robotContoller(double bout[], int numOfRobots, double bin_[][4], double bt_[][3], double b_rs_[][4], double ro, double kkLimits[])
{
    ...
    fcn_A_B(numOfRobots, A, B, bin_, bt_, b_rs_, ro);
/*
    printf("\n");
    mpfr_out_str (stdout, 10, 0, A, MPFR_RND);
    printf("\n");
...
*/
    ...
    */
    ...
    fcn_dAdx(dAdx, MPFR_RND, bin_, bt_);
    fcn_dBdx(numOfRobots, dBdx, B, MPFR_RND, bin_, b_rs_, ro);
    fcn_dAdy(dAdy, MPFR_RND, bin_, bt_);
    fcn_dBdy(numOfRobots, dBdy, B, MPFR_RND, bin_, b_rs_, ro);
